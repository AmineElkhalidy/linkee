...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(AuthCheck, "AuthCheck").$$render($$result, {}, {}, {
    default: () => {
      return `${$userData?.username ? `<h2 class="text-lg sm:text-xl font-semibold text-white -mb-2">Your username is <span class="text-green-500">@${escape($userData.username)}</span></h2> <p class="mb-3" data-svelte-h="svelte-1w50nte">(Usernames cannot be changed)</p> <a href="/login/photo" class="btn btn-primary px-10 text-white font-semibold sm:text-lg" data-svelte-h="svelte-1xyzfcd">Upload Profile Image</a>` : `<form class="w-5/6 sm:w-3/6"><input type="text" placeholder="Username" class="${[
        "input w-full",
